...
//    _password = "tiger";
    ...
    public DBConnectionMgr() throws IOException {
    	...
    	Properties props = new Properties(); // 파일의 내용을 메모리에 올릴 때 사용하는 객체
    	FileInputStream in = new FileInputStream("/Users/Inwook/eclipse-workspace/JspBoard2/WebContent/WEB-INF/dbtest/dbmysql.properties");
    	props.load(in);
    	in.close();
    	
    	
    		_driver=props.getProperty("jdbc.driverClassName");
    		//드라이브 클래스만 시스템에 반영해야해서 바로 아래구문을 써야한다.
    		if(_driver!=null) System.setProperty("jdbc.driverClassName", _driver);
    		...
    		System.out.println("_username:"+_user+" _password:"+_password);
    ...
    public static DBConnectionMgr getInstance() throws Exception {
		...
		if (instance == null) {
            synchronized (DBConnectionMgr.class) {
				...
				if (instance == null) {
					//객체생성
                    instance = new DBConnectionMgr();   //생성자 호출
                }
            }
        }
